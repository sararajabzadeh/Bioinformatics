    ...
    for j in range(n):
        if i==0 and matrix[j][i]!="-":
            ...
            if matrix[j][i]!="-":
                ...
                elif matrix[j][i-1] == "-"and matrix[j][i-2]!="-":
...
for i in probability:
    for j in probability[i]:
        if sumNum[i]!=0 and probability[i][j]!=0:
            ...
            result[result[0].index(i)][result[0].index(j)] = round(probability[i][j]/sumNum[i],3)
...
for i in (alphCount):
    ...
    for j in (alphCount[i]):
        # print(j)
        if alphCount[i][j]!=0 and countalph[i]!=0:
            result2[count][result2[0].index(j)] = round(alphCount[i][j]/countalph[i],3)
